...
class StarModel(Trainer):
    def __init__(
        ...
        output_dir="./starclass_modernbert",
        num_train_epochs=5,
        learning_rate=5e-5,
        batch_size=16,
        max_length=128,
    ...
    def initialize_model(self):
        ...
        self.model = AutoModelForSequenceClassification.from_pretrained(
            self.model_name, num_labels=num_labels, torch_dtype=torch.float32
        )
    ...
    def show_metrics(self):
        ...
        print(f"{label_names} grenade launcher")
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        labels = inputs["label"]
        inputs = {
            key: value for key, value in inputs.items() if key != "label"
        }  # Exclude 'label' key
